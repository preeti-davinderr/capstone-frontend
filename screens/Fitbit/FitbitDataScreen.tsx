...
interface FitbitDataScreenProps {
  accessToken: string;
}

export default function FitbitDataScreen({ accessToken }: FitbitDataScreenProps) {
  const [stepData, setStepData] = useState<string | number | null>(null);
  const [distanceData, setDistanceData] = useState<string | number | null>(null);
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        setStepData(activityJson?.summary?.steps ?? 'N/A');
