...
function convertWeight(value: string, from: 'kg' | 'lbs', to: 'kg' | 'lbs') {
  ...
  return from === 'kg' ? (num * 2.20462).toFixed(1) : (num / 2.20462).toFixed(1);
}
...
export default function WeightInputScreen() {
 ...
 const handleAddEntry = async () => {
  ...
  const monthValue = monthStartWeight ? parseFloat(monthStartWeight.value) : currentValue;
  ...
  return (
    <ScrollView contentContainerStyle={styles.scrollContainer}>
      <View style={styles.headerRow}>
        ...
        <Ionicons name="notifications-outline" size={22} color="#888" style={{ width: 32, textAlign: 'right' }} />
      </View>
      ...
      <View style={styles.card}>
        ...
        <View style={styles.unitSwitchRow}>
          <TouchableOpacity
            style={[styles.unitSwitch, unit === 'kg' && styles.unitSwitchActive]}
            onPress={() => {
              if (unit !== 'kg' && weight) setWeight(convertWeight(weight, 'lbs', 'kg'));
              setUnit('kg');
            }}
          >
            <Text style={[styles.unitSwitchText, unit === 'kg' && styles.unitSwitchTextActive]}>kg</Text>
          </TouchableOpacity>
          <TouchableOpacity
            style={[styles.unitSwitch, unit === 'lbs' && styles.unitSwitchActive]}
            onPress={() => {
              if (unit !== 'lbs' && weight) setWeight(convertWeight(weight, 'kg', 'lbs'));
              setUnit('lbs');
            }}
          >
            <Text style={[styles.unitSwitchText, unit === 'lbs' && styles.unitSwitchTextActive]}>lbs</Text>
          </TouchableOpacity>
        </View>
      ...
      <View style={styles.rowStats}>
        <View style={styles.statBox}>
          <Text style={styles.statValue}>{current ? `${current.value} ${current.unit}` : '--'}</Text>
      ...
      <View style={styles.card}>
        ...
        {displayHistory.map((item, idx) => (
          <View style={styles.historyItem} key={idx}>
            <View>
              <Text style={styles.historyWeight}>{item.value} {item.unit}</Text>
              <Text style={styles.historyDate}>{getTodayLabel(item.date)}</Text>
            </View>
